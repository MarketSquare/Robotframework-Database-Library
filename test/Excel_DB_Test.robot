*** Settings ***
#Suite Setup      log to console   ${DBName}
Suite Setup       Connect To Database  excelrw   ${DBName}    ${DBUser}    ${DBPass}    ${DBHost}    ${DBPort}
...
Execute SQL Script - Insert Data person table
    ...
    Comment    ${output} =    Execute SQL Script    ${EXECDIR}/test//${DBName}_insertData.sql
    ${output} =    Execute SQL Script    ${EXECDIR}/test//my_db_test_insertData.sql
    Log    ${output}
    Should Be Equal As Strings    ${output}    None
